...
from thermompnn_model import TransferModel
from datasets import FireProtDataset, MegaScaleDataset, MdrDataset, ComboDataset
...
def train(cfg):
    ...
    if 'project' in cfg:
        wandb.init(project=cfg.project, name=cfg.name, mode="offline")
    else:
        cfg.name = 'test'
    ...
    if len(cfg.datasets) == 1: # one dataset training
        ...
        if dataset == 'fireprot':
            ...
            train_dataset = MdrDataset(cfg.data_loc.mdrdb_tsv, cfg.data_loc.mdrdb_splits, "train", cfg.data_loc.mdrdb_pdbs, ["Single Substitution"], ["all"])
            val_dataset = MdrDataset(cfg.data_loc.mdrdb_tsv, cfg.data_loc.mdrdb_splits, "val", cfg.data_loc.mdrdb_pdbs, ["Single Substitution"], ["all"])
        else:
    ...
    try:
        model_pl = TransferModelPL.load_from_checkpoint(cfg.platform.model_checkpoint, cfg=cfg)
    except:
